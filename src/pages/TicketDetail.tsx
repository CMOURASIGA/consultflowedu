...
export function TicketDetailPage() {
  ...
  const fetchTicket = () => {
    ...
    const ticket = savedTickets.find((t: any) => t.id === id || String(t.id) === String(id));
    
    if (ticket) {
      // Create a mock dataset for the ticket
      setData({
        ticket: {
          ...ticket,
          original_message: ticket.description || 'Nenhuma descrição fornecida',
          summary: 'Resumo gerado (Mock)',
          sentiment: 'Neutro'
        },
        messages: []
      });
      ...
      // Fallback to API if not in local storage, or just mock it

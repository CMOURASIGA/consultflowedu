...
export function DashboardPage() {
  ...
  useEffect(() => {
    // In a real app we'd fetch stats from the server
    fetch('/api/tickets')
      .then(res => res.json())
      .then(data => {
        if (Array.isArray(data)) {
          setStats({
            total: data.length,
            delayed: data.filter(t => t.due_at && new Date(t.due_at) < new Date() && t.status !== 'resolvido').length,
            resolved: data.filter(t => t.status === 'resolvido').length,
